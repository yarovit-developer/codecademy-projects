...
const keyReturn = function(event) {
  event.target.style.backgroundColor = ''; // Reset to the original color
}

// Write a named function with event handler properties
let eventAssignment = function(note) {
  note.onmousedown = function(event) {
    keyPlay(event); // Передаем event в функцию keyPlay
  }
  note.onmouseup = function(event) {
    keyReturn(event); // Передаем event в функцию keyReturn
  }
}
